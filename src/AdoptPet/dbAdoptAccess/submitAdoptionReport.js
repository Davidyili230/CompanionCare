...
export async function submitAdoptionReport(formData) {
    const auth = getAuth();
    ...
    try {
        ...
        if (formData.imageFile) {
            const imageRef = ref(storage, `AdoptionPets/${formData.name + v4()}`);
        ...
        await addDoc(collection(db, "AdoptionPetReports"), data);
